...
module.exports = {
  ...
  deploy: {
    ...
    client: {
      ...
      path: "/var/www/stage/" + client_name + "/" + project_name,
      "post-deploy":
        "npm config set @fortawesome:registry https://npm.fontawesome.com/EEFD84F8-907F-4B20-82D9-DC20387F3808 && npm install && npm run build && rm -r build && mv public build && cp -r ./build/* ../htdocs_client/",
      env: {
        NODE_ENV: "client",
      },
    },
    qa: {
      ...
      path: "/var/www/stage/" + client_name + "/" + project_name,
      "post-deploy":
        "npm config set @fortawesome:registry https://npm.fontawesome.com/EEFD84F8-907F-4B20-82D9-DC20387F3808 && npm install && npm run build && rm -r build && mv public build && cp -r ./build/* ../htdocs_qa/",
      env: {
        NODE_ENV: "qa",
      },
    },
